...
query_task = Task(
    description=(
        "The patient query is: {patient_query}. "
...
while True:
    ...
    if user_input.startswith("doc:"):
        # Route to Medical Document Summarizer
        doc_text = user_input.replace("doc:", "").strip()
        result = summarizer_crew.kickoff(inputs={"document_text": doc_text})
    else:
        # Route to Query Classifier Agent
        result = query_crew.kickoff(inputs={"patient_query": user_input})
